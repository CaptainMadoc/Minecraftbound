...
function realInit(dt)
	...
	data["uiShell"] = "/gunsboundminecraft/ui/ammowood.png"
	data["inAccuracy"] = 1
...
function realUpdate(dt)
	...
	--ammo Display
	local matt = {
		{1 / data["maxMagazine"],0,0},
		{0,1,0},
		{0,0,1}
	}
	local shift = (-0.25) * (30 / data["maxMagazine"])
	...
	if data["load"] == "table" then
		localAnimator.addDrawable({image = data["uiShell"], position = vec2.add(position, {0, -2.75}), transformation = matt, fullbright = true, color = {255,255 / ratio1,255 / ratio1,255}}, "overlay")
		shift = 0
